        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($registers)
                ->editColumn('total_card_slips', function ($row) {
                    return $this->transactionUtil->num_f($row->total_card_payment, true) . ' (' . $row->total_card_slips . ')';
                })
                ->editColumn('total_cheques', function ($row) {
                    return $this->transactionUtil->num_f($row->total_cheque_payment, true) . ' (' . $row->total_cheques . ')';
                })
                ->editColumn('total_cash_payment', function ($row) {
                    return $this->transactionUtil->num_f($row->total_cash_payment, true);
                })
                ->editColumn('total_bank_transfer_payment', function ($row) {
                    return $this->transactionUtil->num_f($row->total_bank_transfer_payment, true);
                })
                ->editColumn('total_other_payment', function ($row) {
                    return $this->transactionUtil->num_f($row->total_other_payment, true);
                })
                ->editColumn('total_advance_payment', function ($row) {
                    return $this->transactionUtil->num_f($row->total_advance_payment, true);
                })
                ->editColumn('total_custom_pay_1', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_1, true);
                })
                ->editColumn('total_custom_pay_2', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_2, true);
                })
                ->editColumn('total_custom_pay_3', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_3, true);
                })
                ->editColumn('total_custom_pay_4', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_4, true);
                })
                ->editColumn('total_custom_pay_5', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_5, true);
                })
                ->editColumn('total_custom_pay_6', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_6, true);
                })
                ->editColumn('total_custom_pay_7', function ($row) {
                    return $this->transactionUtil->num_f($row->total_custom_pay_7, true);
                })
